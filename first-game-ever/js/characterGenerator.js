...
function createCharacter(playerSpeedInPx) {
  ...
  this.spriteSheet = {
    ...
    idle: new createSprite(
      ...
      -9,
      -40, // x and y pos of animation (top left corner), for 100% scale of sprite: 3, -11
      8,
      4, // rows, columns
    ...
    run: new createSprite(
      ...
      -9,
      -40, // x and y pos of animation (top left corner)
      8,
      8, // rows, columns
    ...
    if (r >= 0 && r < m.tiles.length) {
      for (c = cm.area.left; c < m.tiles[r].length - cm.area.right; c++) {
        if (c >= 0 && c < m.tiles[r].length) {
          if (
            m.tiles[r][c].x > cm.area.points.topLeft.x &&
...
createCharacter.prototype.upX = function (m, p) {
  ...
  p.collisionModel.area.left = Math.floor(
    p.collisionModel.area.points.topLeft.x / m.tileWidth
  );
  p.collisionModel.area.points.bottomRight.x += p.xV;
  p.collisionModel.area.right = Math.floor(
    (m.mapWidth - p.collisionModel.area.points.bottomRight.x) / m.tileWidth
  );
};
createCharacter.prototype.upY = function (m, p) {
  ...
  p.collisionModel.area.top = Math.floor(
    p.collisionModel.area.points.topLeft.y / m.tileHeight
  );
  p.collisionModel.area.points.bottomRight.y += p.yV;
  p.collisionModel.area.bottom = Math.floor(
    (m.mapHeight - p.collisionModel.area.points.bottomRight.y) / m.tileHeight
  );
};
...
createCharacter.prototype.calcCollisionModel = function (
  mapObject,
  playerObject
) {
  ...
  */
  ...
  this.area = {
    ...
    top: Math.floor(
      (playerObject.y - this.areaDimensions.height) / mapObject.tileHeight
    ),

    right: Math.floor(
      (mapObject.mapWidth - (playerObject.x + this.areaDimensions.width)) /
        mapObject.tileWidth
    ),

    bottom: Math.floor(
      (mapObject.mapHeight - playerObject.y - this.areaDimensions.width) /
        mapObject.tileHeight
    ),

    left: Math.floor(
      (playerObject.x - this.areaDimensions.width) / mapObject.tileWidth
    ),
  };
...
createCharacter.prototype.move = function (m, p) {
  ...
  if (p.z < l.z) {
    ...
    if (p.zV !== 0) {
      // this prevents repeating zeroing operation if not neccesary
      p.zV = 0;
    }
    p.isInTheAir = false;
    p.isJumping = false;
    p.jumpDelayCounter =
      p.jumpDelayCounter - 1 >= 0 ? p.jumpDelayCounter - 1 : 0;
  }
  ...
  if (37 in keysDown) {
    ...
    if (
      p.x > m.clipByTiles * m.tileWidth + p.xV + p.spriteWidth / 2 &&
      p.x < m.mapWidth - m.clipByTiles * m.tileWidth
    ) {
      // Move player
      p.xV -= p.speed;
      if (p.isJumping) {
  ...
  if (39 in keysDown) {
    // Move player if he's in the range of map
    if (
      p.x > m.clipByTiles * m.tileWidth + p.xV &&
      p.x < m.mapWidth - m.clipByTiles * m.tileWidth - p.spriteWidth / 2
    ) {
      // Move player
      p.xV += p.speed;
      if (p.isJumping) {
  ...
  if (38 in keysDown) {
    ...
    if (
      p.y >
        m.clipByTiles * m.tileHeight + p.yV + p.spriteHeight + p.spriteHeight &&
      p.y < m.mapHeight - m.clipByTiles * m.tileWidth
    ) {
      // Move player
      p.yV -= p.speed;
      if (p.isJumping) {
