...
generateMap.prototype.render = function (playerObject) {
  ...
  let top =
      m.tilesOutsideCanvas
        .top /*- Math.floor( Math.abs( m.zMin ) / m.tileHeightHalf )*/ - 3,
    bottom =
      m.yTilesNumber /*- m.tilesOutsideCanvas.bottom + Math.floor( Math.abs( m.zMax ) / m.tileHeightHalf )*/ +
      3,
  ...
  if (map.renderSwitch) {
    ...
    for (r = top; r < bottom + 3; r++) {
      if (r >= 0 && r < m.tiles.length) {
        if (
          m.tiles[r][0].y <= p.y ||
          (p.y >= m.tiles[r][0].y && p.y <= m.tiles[r][0].y + m.tileHeightHalf)
        ) {
          for (c = left - 3; c < right + 3; c++) {
            if (
              ...
              m.tiles[r][c].y + m.tiles[r][c].z <
                o + canvasHeight + m.tileHeight
            ) {
              ctx1.drawImage(
                m.tiles[r][c].type,
      ...
      if (r >= 0 && r < m.tiles.length) {
        for (c = left - 3; c < right + 3; c++) {
          if (
            ...
            if (
              (m.tiles[r][c].z < p.z &&
                r === Math.floor(p.y / m.tileHeightHalf)) ||
              r > Math.floor(p.y / m.tileHeightHalf)
            ) {
              ctx1.drawImage(
                m.tiles[r][c].type,
